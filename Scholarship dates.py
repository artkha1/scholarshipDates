...
wks = gc.open("Scholarship Application Organizer (Red = apply early, Orange = Last Year's Deadline)").sheet1
...
def create_event(name,date): #date is yyyy-mm-dd
 ...
 #Found it out by experimenting and removing that parameter in the "try it" section of https://developers.google.com/calendar/create-events
  ...
  'reminders': {
    'useDefault': False,
    'overrides': [{'method': 'email', 'minutes': 120 * 60}] #remind 5 days before
  },
}
...
cell_list = wks.range(f'J2:J{last}') #Event Created column. Don't forget to change the letter J to something else if I remove some columns
